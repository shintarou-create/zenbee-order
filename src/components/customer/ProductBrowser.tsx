...
interface ProductBrowserProps {
  ...
  // 「カートを見る」ボタンの遷移先（本番="/cart"／プレビュー="/admin/preview/cart"）
  cartHref: string
}

// 商品一覧（カテゴリアコーディオン→数量入力→カート追加）の画面ロジック。
// 本番LIFF発注画面（src/app/page.tsx）と管理画面の発注画面プレビュー
// （src/app/admin/preview/page.tsx）の両方から使う共有コンポーネント。
export default function ProductBrowser({
  ...
  const { items: cartItems, addToCart, addCustomItem, itemCount, customItemCount } = useCart(cartStorageKey)
