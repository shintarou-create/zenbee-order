...
function orderToRows(order: OrderForCsv): string[][] {
  ...
  function buildRow(
    ...
    return [
      getTodayJSTString(),                         //  1: 出荷予定日（今日のJST日付）
      orderNum,                                    //  2: お客様管理番号
      isMultiPackage ? '6' : '0',                  //  3: 送り状種類（6=複数口 / 0=発払い）
      String(coolType),                            //  4: クール区分
      '',                                          //  5: お届け先コード
      company.phone.replace(/-/g, ''),             //  6: お届け先電話番号
      '',                                          //  7: お届け先電話番号枝番
      company.representativeName,                  //  8: お届け先名
      company.postalCode.replace('-', ''),         //  9: お届け先郵便番号
      recipientAddress,                            // 10: お届け先住所
      recipientBuilding,                           // 11: お届け先建物名
      company.companyName,                         // 12: お届け先会社・部門名１
      '',                                          // 13: お届け先会社・部門名２
      '',                                          // 14: お届け先名略称カナ
      '',                                          // 15: 敬称
      getYamatoCustomerCode(),                     // 16: ご依頼主コード
      sender.phone,                                // 17: ご依頼主電話番号
      '',                                          // 18: ご依頼主電話番号枝番
      sender.name,                                 // 19: ご依頼主名
      sender.postalCode,                           // 20: ご依頼主郵便番号
      sender.address,                              // 21: ご依頼主住所
      sender.building,                             // 22: ご依頼主建物名
      '',                                          // 23: ご依頼主名略称カナ
      '',                                          // 24: 品名コード１
      itemName,                                    // 25: 品名１
      '',                                          // 26: 品名コード２
      ...
      handling2,                                   // 29: 荷扱い２
      '',                                          // 30: 記事
      deliveryDate,                                // 31: お届け予定（指定）日
      toYamatoTimeSlotCode(order.deliveryTimeSlot),// 32: 配達時間帯区分（ヤマトコード）
      '',                                          // 33: コレクト代金引換額（税込）
      '',                                          // 34: コレクト内消費税額等
      '',                                          // 35: 営業所止置き
      '',                                          // 36: 営業所コード
      isMultiPackage ? '3' : '',                   // 37: 個数口枠の印字
      String(boxCount),                            // 38: 発行枚数
      getYamatoCustomerCode(),                     // 39: ご請求先顧客コード
      '',                                          // 40: ご請求先分類コード
      getYamatoFreightManagementNo(),              // 41: 運賃管理番号
      '',                                          // 42: お届け予定ｅメール利用区分
      '',                                          // 43: お届け予定ｅメールアドレス
      '',                                          // 44: 入力機種
      '',                                          // 45: お届け予定ｅメールメッセージ
      ...
      '',                                          // 47: お届け完了ｅメールアドレス
      '',                                          // 48: お届け完了ｅメールメッセージ
      isMultiPackage ? orderNum : '',              // 49: 複数口くくりキー（複数口時のみ注文番号）
    ]
  }
